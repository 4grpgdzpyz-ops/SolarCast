...
struct SettingsDTO: Codable, Sendable, Equatable {
    ...
    // logRetentionDays follows the same real pattern, defaulting to 3
    // (matching AppLogger's own real default) for any backup exported
    ...
    init(from decoder: Decoder) throws {
        ...
        logRetentionDays = try c.decodeIfPresent(Int.self, forKey: .logRetentionDays) ?? 3
    }
}
