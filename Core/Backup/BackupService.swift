...
actor BackupService {
    ...
    func exportSettings() -> SettingsDTO {
        ...
        return SettingsDTO(
            ...
            autoFetchTiming: .from(config.autoFetchTiming), logRetentionDays: AppLogger.shared.retentionDays)
    }
    ...
    private func applySettings(_ s: SettingsDTO) {
        ...
        AppLogger.shared.retentionDays = s.logRetentionDays
    ...
    private func importSwiftData(apiKeys: [APIKeyDTO], pvSites: [PVSiteDTO], location: LocationDTO?,
        ...
        // Batch-fetch every existing API key ONCE, upfront, into a real,
        // in-memory dictionary keyed by id — replacing what was
        // previously one individual, synchronous FetchDescriptor query
        // PER imported key. API keys are few, so this specific loop's
        // own real cost was always small — but the SAME pattern below,
        // for forecast points, is where the real, meaningful improvement
        // lives, since that's the dominant volume in a real backup.
        let existingKeys = (try? ctx.fetch(FetchDescriptor<APIKeyEntity>())) ?? []
        var existingKeysByID: [UUID: APIKeyEntity] = Dictionary(uniqueKeysWithValues: existingKeys.map { ($0.id, $0) })

        var keyEntityMap: [UUID: APIKeyEntity] = [:]
        for k in apiKeys {
            let keyValue = decrypt(k.keyValueEncrypted)
            if let existing = existingKeysByID[k.id] {
                ...
                existingKeysByID[k.id] = entity
            }
        }

        // Same, real batch-fetch pattern for PV sites — one query,
        // upfront, instead of one per imported site.
        let existingSites = (try? ctx.fetch(FetchDescriptor<PVSiteEntity>())) ?? []
        var existingSitesByID: [UUID: PVSiteEntity] = Dictionary(uniqueKeysWithValues: existingSites.map { ($0.id, $0) })

        var siteEntityMap: [UUID: PVSiteEntity] = [:]
        for s in pvSites {
            let solcastID = decrypt(s.solcastSiteIDEncrypted)
            if let existing = existingSitesByID[s.id] {
        ...
        if let points = forecastPoints {
            // The real, dominant fix — one batch fetch of ALL existing
            // forecast points, upfront, instead of one individual query
            // PER imported point. For a real backup spanning weeks or
            // months, this is the difference between one database
            // round-trip and thousands.
            let existingPoints = (try? ctx.fetch(FetchDescriptor<ForecastPointEntity>())) ?? []
            var existingPointsByID: [String: ForecastPointEntity] = Dictionary(uniqueKeysWithValues: existingPoints.map { ($0.pointID, $0) })

            for p in points {
                let siteEntity = siteEntityMap[p.pvSiteID]
                if let existing = existingPointsByID[p.pointID] {
