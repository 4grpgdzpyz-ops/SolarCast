...
final class BGTaskCoordinator {
    // Derived from the real, actual running bundle identifier, rather
    // than hardcoded — matching Info.plist's own
    // $(PRODUCT_BUNDLE_IDENTIFIER) substitution there, so these two
    // real, separate declarations (this one, and the one in
    // BGTaskSchedulerPermittedIdentifiers) can never genuinely drift
    // apart, regardless of which build configuration (debug or
    // release, each with its own distinct, real bundle identifier) is
    // actually running. A real, genuine mismatch here is exactly what
    // produces BGTaskSchedulerErrorDomain Code=3 ("Unrecognized
    // Identifier") at submission time.
    private static let bundleID = Bundle.main.bundleIdentifier!
    static let refreshTaskID = "\(bundleID).refresh"
    static let maintenanceTaskID = "\(bundleID).maintenance"
