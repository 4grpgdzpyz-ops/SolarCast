...
/// Central scheduling authority with four responsibilities:
///
...
///
/// 2. nextMaintenance(): the next daily maintenance trigger — genuinely
///    unconditional and deterministic (always the next UTC midnight),
///    entirely independent of nextScheduledFetch()'s own fetch/refresh
///    competition, since maintenance never had anything real to compete
///    against.
///
/// 3. staleAPIKeys(): used at app launch / foreground resume to decide
///    which keys need an immediate catch-up fetch, delegating the actual
///    per-key staleness rules to StalenessEvaluator.
///
/// 4. Owns the StalenessEvaluator instance internally, constructed from
...
actor SchedulingEngine {
    ...
    /// Returns the next real trigger for EACH of the two, genuinely
    /// independent BGTaskScheduler identifiers this app now uses —
    /// refresh (fetch/refresh) and maintenance (cleanup) — rather than
    /// one, single, combined winner. This is the real, structural fix
    /// for a confirmed bug: when both real task families competed in
    /// ONE min() comparison, whichever family didn't win was silently
    /// never submitted that cycle at all, even when genuinely enabled —
    /// sometimes leaving one or even both real identifiers without a
    /// currently pending request. Maintenance's own candidate is
    /// computed and returned entirely separately from fetch/refresh's,
    /// so BGTaskCoordinator.scheduleNext can genuinely, independently
    /// submit to both real identifiers every cycle, matching how they
    /// actually, structurally work now.
    /// The next fetch/refresh trigger — genuinely the only real
    /// candidates that ever compete for the single, real "refresh"
    /// BGTaskScheduler slot. Maintenance is deliberately NOT part of
    /// this competition at all (see nextMaintenance() below) — it's an
    /// unconditional, deterministic value with nothing to "win" against.
    ...
    /// The next daily maintenance trigger — genuinely unconditional and
    /// deterministic (always the next UTC midnight), with no real
    /// competition involved at all. Quota-usage cleanup always runs as
    /// part of it; log cleanup within it is separately, further gated
    /// on the logging toggle inside BGTaskCoordinator's own handler, not
    /// here — this function's only real job is "when is the next UTC
    /// midnight," nothing more.
    func nextMaintenance(now: Date = Date()) -> ScheduledTrigger {
        let t = AppLogger.shared.nextCleanupBoundary(now: now)
        AppLogger.shared.info("SchedulingEngine: daily maintenance scheduled for \(t)")
        return .logCleanup(t)
    }
    ...
    private static func pickWinner(_ candidates: [ScheduledTrigger]) -> ScheduledTrigger? {
        ...
        if let winner {
            ...
            case .logCleanup:
                // Genuinely unreachable — this function is only ever
                // called with candidates built exclusively from
                // .fetch/.refresh appends; maintenance has its own,
                // separate, unconditional computation (nextMaintenance())
                // entirely outside this candidate/winner machinery now.
                break
            }
        } else {
            AppLogger.shared.info("SchedulingEngine: nextScheduledFetch produced no candidates at all (auto-fetch and auto-refresh are both disabled, or neither produced a valid trigger)")
        }
        return winner
    }
