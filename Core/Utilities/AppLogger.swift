...
final class AppLogger: @unchecked Sendable {
    ...
    private static let retentionDaysKey = "solarcast.logRetentionDays"
    ...
    /// How many real, actual days of log files to retain before cleanup
    /// deletes them — user-configurable, 1 to 7, default 3. Clamped on
    /// both read and write so a malformed or out-of-range stored value
    /// (e.g. from a future version's own default changing, or manual
    /// UserDefaults tampering) can never produce a genuinely invalid
    /// retention window.
    var retentionDays: Int {
        get {
            let stored = UserDefaults.standard.object(forKey: Self.retentionDaysKey) as? Int
            return min(max(stored ?? 3, 1), 7)
        }
        set { UserDefaults.standard.set(min(max(newValue, 1), 7), forKey: Self.retentionDaysKey) }
    }
    ...
    /// Removes log files older than the user's configured retentionDays.
    func cleanupOldLogs() {
...
extension Notification.Name {
    ...
    /// via BGTaskCoordinator.handleRefresh/handleMaintenance, or a stale-check-triggered
