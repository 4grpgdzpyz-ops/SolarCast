...
actor QuotaManager {
    ...
    /// Records real usage against a key's quota — but only if the key
    /// isn't already at or past its own daily limit. A failed call still
    /// genuinely happened and consumed a real Solcast API call server-side
    /// (see consumedRealCall's own doc), so this isn't about pretending
    /// the attempt didn't occur — it's about keeping the LOCAL, derived
    /// "used" count meaningful once it's already reached the real limit,
    /// rather than letting it climb past dailyQuotaLimit (e.g. 11/10)
    /// purely because multiple sites' own, separate calls can each
    /// independently record a failure in quick succession, before any one
    /// of them has a chance to react to the others' own outcome.
    func recordUsage(apiKey: APIKey, wasSuccessful: Bool, purpose: FetchPurpose, isMock: Bool, consumedRealCall: Bool = true, now: Date = Date()) async throws {
        if !apiKey.hasUnlimitedQuota {
            let used = try await windowTracker.usedSinceUTCMidnight(apiKeyID: apiKey.id, now: now)
            guard used < apiKey.dailyQuotaLimit else {
                AppLogger.shared.info("QuotaManager: recordUsage skipped for key '\(apiKey.name)' — already at or above limit (used=\(used), limit=\(apiKey.dailyQuotaLimit))")
                return
            }
        }
        try await quotaRepository.recordUsage(
            QuotaUsageEvent(apiKeyID: apiKey.id, timestamp: now, wasSuccessful: wasSuccessful, purpose: purpose, isMock: isMock, consumedRealCall: consumedRealCall))
    }
