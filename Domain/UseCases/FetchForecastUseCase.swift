...
actor FetchForecastUseCase {
    /// Set true for the duration of executeAutoFetch/executeAutoRefresh,
    /// cleared via defer on every exit path. executeAppLaunchIfStale
    /// checks this at its own start and skips entirely if true — this is
    /// the real, direct fix for a confirmed, real race: a background job
    /// and an app-launch staleness check could previously both read
    /// quota's "used so far" count before either had recorded its own
    /// usage, letting the combined total exceed the daily limit even
    /// though each individual check was, in isolation, correct.
    private var isBackgroundJobInProgress = false
    ...
    func executeAutoFetch() async throws {
        isBackgroundJobInProgress = true
        defer { isBackgroundJobInProgress = false }
        try await execute(purpose: .autoFetch)
    }
    ...
    func executeAppLaunchIfStale(schedulingEngine: SchedulingEngine, bgTaskCoordinator: BGTaskCoordinator, onWillFetch: (() async -> Void)? = nil) async throws -> StalenessCheckResult {
        // A background job (auto-fetch or auto-refresh) is already
        // running — skip the staleness check entirely rather than risk
        // both independently reading the same, real "used so far" quota
        // count before either has recorded its own usage, which could
        // let their combined total exceed the daily limit.
        guard !isBackgroundJobInProgress else { return .notStale }
    ...
    private func execute(purpose: FetchPurpose, restrictToSiteIDs: [UUID]? = nil, now: Date = Date()) async throws {
        ...
        for result in results {
            ...
            case .success(let sid, let dtos):
                ...
                try await quotaManager.recordUsage(apiKey: job.apiKey, wasSuccessful: true, purpose: purpose, isMock: isMock, now: now)
            ...
            case .failure(let sid, let fetchError):
                ...
                try await quotaManager.recordUsage(apiKey: job.apiKey, wasSuccessful: false, purpose: purpose, isMock: isMockFailure, consumedRealCall: consumedRealCall, now: now)
    ...
    private func filterIncompleteLastDay(now: Date) async {
        do {
            ...
            // Compare against that specific day's actual, real sunset — not
            // a fixed, generic clock time. A fixed hour is wrong in both
            // directions depending on season and location: it could keep a
            // day that's still missing real daylight hours in summer (when
            // sunset is later), or discard a day that's genuinely complete
            // in winter (when sunset is earlier).
            guard let location = try? await locationRepository.fetchCurrent() else { return }
            guard let sunWindow = await sunWindowCalculator.resolve(date: lastPoint.periodEnd, location: location)
            else { return }
            if lastPoint.periodEnd < sunWindow.sunset {
                AppLogger.shared.info("Filtering incomplete last forecast day: \(lastDate) (last point: \(lastPoint.periodEnd), sunset: \(sunWindow.sunset))")
