...
struct DashboardView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .alert("Data Refreshed", isPresented: $viewModel.showManualRefreshSuccess) {
                Button("OK") {}
            } message: {
                Text("Your data has been refreshed.")
            }
            .alert("Refresh Failed", isPresented: Binding(
                ...
                Text("Your forecast data was out of date, but refreshing it failed: \(staleDataFetchErrorMessage ?? "Unknown error.")")
            }
            .task { await viewModel.loadAll() }
            .onReceive(NotificationCenter.default.publisher(for: .forecastDataRefreshed)) { notification in
                Task {
                    viewModel.isRefreshing = true
                    await viewModel.loadAll()
                    viewModel.isRefreshing = false
                    if let errorMessage = notification.userInfo?["errorMessage"] as? String {
                        staleDataFetchErrorMessage = errorMessage
                    } else {
                        showStaleDataNotice = true
                    }
                }
