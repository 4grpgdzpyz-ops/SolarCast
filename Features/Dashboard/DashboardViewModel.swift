...
@Observable final class DashboardViewModel {
    ...
    var showManualRefreshSuccess = false
    ...
    func loadDatesWithData() async {
        do {
            // Fetch all points across an unbounded historical range (all
            // real, stored past data) plus a 14-day forecast — previously
            // capped at 30 days back, which meant genuinely stored,
            // exportable historical data older than that silently never
            // showed as available in the calendar picker, even though it
