...
final class AppDelegate: NSObject, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication,
        ...
        // Only run the launch-time staleness check if this is a genuine,
        // real, user-initiated open — not an OS-initiated launch to run a
        // background task. When iOS launches the app process specifically
        // for a scheduled BGTaskScheduler job, the app genuinely launches
        // into .background state, not .active — the background task's own
        // logic (BGTaskCoordinator.handleRefresh) already, independently
        // checks staleness and fetches if needed, so running the same
        // check here too would just be a real, duplicate fetch for the
        // same underlying reason, not two genuinely separate, valid
        // triggers.
        if application.applicationState == .active {
            Task {
                ...
                case .fetchFailed(let error):
                    ...
                    // logged here AND posts the same notification with the
                    // error's message in userInfo, so DashboardView's existing
                    // observer can show the same real, honest "Refresh Failed"
                    // alert the resume path already provides for this case —
                    // AppDelegate itself still has no UI capability to show an
                    // alert directly.
